...
for img,trials in trials_by_img.iteritems():
    ...
    if im2lab[img+'.jpg'] == 'animal':
        model_comp['animal']['human_acc'][0][im2key[img+'.jpg']] = float(score*100)
    else:
        model_comp['nonanimal']['human_acc'][0][im2key[img+'.jpg']] = float(score*100)
...
#P.plot(mrt_500_avgrt, 'b',label='500ms Max RT')
#P.hold = True
#P.plot(range(1,50),trend(range(1,50)), 'k')
#P.hold = True
#P.plot(mrt_1000_avgrt, 'r',label='1000ms Max RT')
#P.hold = True
#P.plot(range(1,50),trend1(range(1,50)), 'k')
#P.hold = True
#P.plot(mrt_2000_avgrt, 'g',label='2000ms Max RT')
#P.plot(range(1,50),trend2(range(1,50)), 'k')
...
#trend2 = np.poly1d(fit)
#P.plot(mrt_500_avgclass, 'b',label='500ms Max RT')
#P.hold = True
#P.plot(range(1,50),trend(range(1,50)), 'k')
#P.hold = True
#P.plot(mrt_1000_avgclass, 'r',label='1000ms Max RT')
#P.hold = True
#P.plot(range(1,50),trend1(range(1,50)), 'k')
#P.hold = True
#P.plot(mrt_2000_avgclass, 'g',label='2000ms Max RT')
#P.plot(range(1,50),trend2(range(1,50)), 'k')
...
def overviewPlot(model, corr, by_mrt,shouldCombine, feats, total_acc,abstract, n_subj):
    ...
    if (by_mrt):
        ...
        if not(abstract):
            ...
            ax1.plot([-0.5,n_layers-0.5],[np.mean(total_acc)*100,int(np.mean(total_acc)*100)],color='0.5',ls='--')
            ...
            #bootstrap CIs
            confs = bootstrap(1000,.05,feats,im2lab,n_subj)
            pickle.dump(confs,open('conf_vals.p','wb'))
            p3_fit = np.polyfit(xrange(n_layers),confs[0][:],3)
            p3_fn = np.poly1d(p3_fit)
            xs = np.linspace(0, n_layers-1)
            ax2.plot(xs,p3_fn(xs),'#ffb3b3')
            print confs
            ax2.scatter(xrange(n_layers),confs[1][:])
            ax2.scatter(xrange(n_layers),confs[0][:])
            ...
            plt.sca(ax2)
            plt.xticks(range(n_layers),['conv1_1', 'conv1_2', 'conv2_1', 'conv2_2', 'conv3_1', 'conv3_2', 'conv3_3', 'conv4_1', 'conv4_2', 'conv4_3', 'conv5_1', 'conv5_2', 'conv5_3','fc6','fc7'],rotation=180)
            plt.sca(ax1)
...
#plot_corr_correct(VGG16_feats['VGG16_fc7ex'], 'VGG16', 'fc7ex',  corrs[0],False,False,False)
...
overviewPlot('VGG16', corrs[0], True, False,VGG16_feats,total_acc,False,77)
#overviewPlot('VGG16', corrs[0], False,False,VGG16_feats, total_acc)
